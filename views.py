...
def cart_detail(request,total=0,counter=0,cart_items=None,price=None):
    if request.user.is_authenticated:
        try:
            ...
            pass
        return render(request,'shopping/cart.html',dict(user=request.user,cart_items=cart_items,total=total,counter=counter, price=price))
    return redirect('login')
...
def checkout(request, total=0, counter=0, cart_items=None):
    if request.user.is_authenticated:
        ...
        return render(request, 'shopping/checkout.html',
                      dict(user=request.user, cart_items=cart_items, total=total, counter=counter, price=price))
    return redirect('login')
