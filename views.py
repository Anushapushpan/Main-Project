...
def cart_detail(request,total=0,counter=0,cart_items=None,sum=0):
    if request.user.is_authenticated:
        try:
            cart=Cart.objects.get(cart_id=_cart_id(request))
            ...
            pass
        return render(request,'shopping/cart.html',dict(user=request.user,cart_items=cart_items,total=total,sum=sum,counter=counter))
    return redirect('login')
...
def checkout(request, total=0, counter=0, cart_items=None):
    if request.user.is_authenticated:
        ...
        return render(request, 'shopping/checkout.html',
                        dict(user=request.user, cart_items=cart_items, total=total, counter=counter))
    return redirect('login')
